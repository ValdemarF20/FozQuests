...
@Getter
@ConfigSerializable
public class IconMenuSettings implements LocaleReference {
    @Comment("The title used for the menu")
    @Setting
    private final Component title = ChatUtil.deserialize("<blue>Quest Icon Configuration Menu");

    @Comment("The item used in \"/q create\" as button for icon")
    @Setting
    private final ItemStack template = ItemBuilder.from(Material.PAPER)
            .name(ChatUtil.deserialize("Change icon item"))
            .lore(
                    ChatUtil.deserialize(""),
                    ChatUtil.deserialize("<green>Configure the icon")
            ).build();

    @Comment("The name configurator for the icon")
    @Setting
